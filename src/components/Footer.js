import '../App.css';
import "../css/Footer.css";
export default function Footer(){
    return (
        <footer>
            <div>
                <h1 class="footer-head">Doormat Navigation</h1>
                <ul class="footer-section">
                    <li><a href="https://www.google.ca/">Home</a></li>
                    <li><a href="https://www.google.ca/">About</a></li>
                    <li><a href="https://www.google.ca/">Menu</a></li>
                    <li><a href="https://www.google.ca/">Reservations</a></li>
                    <li><a href="https://www.google.ca/">Order Online</a></li>
                    <li><a href="https://www.google.ca/">Login</a></li>
                </ul>
            </div>
            <div>
                <h1 class="footer-head">Contact</h1>
                <ul class="footer-section">
                    <li><a href="https://www.google.ca/">Address</a></li>
                    <li><a href="https://www.google.ca/">Phone Number</a></li>
                    <li><a href="https://www.google.ca/">Email</a></li>
                </ul>
            </div>
            <div>
                <h1 class="footer-head">Social Media Links</h1>
                <ul class="footer-section">
                    <li><a href="https://www.google.ca/">Address</a></li>
                    <li><a href="https://www.google.ca/">Phone Number</a></li>
                    <li><a href="https://www.google.ca/">Email</a></li>
                </ul>
            </div>
        </footer>
    );
}